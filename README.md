...
1. Local tracking, GOTO, follow-target, and jog state are cleared.
2. RA and DEC remain enabled.
3. `AccelStepper::stop()` is applied to both axes, so the motors decelerate using the configured acceleration.
4. When both axes reach zero `distanceToGo()`, the firmware saves the reached RA/DEC into `RES_CURRENT_RA_*` and `RES_CURRENT_DEC_*`.
5. `RES_STATUS` returns to `IDLE`.

STM32 does not rewrite `REQ_TRACKING_ENABLE` or any other request register while processing STOP. Request registers remain ESP32-owned; the stopped state is reported through `RES_STATUS=IDLE`.
